...
- [x] Task 4: Setup Session Management & Protected Routes (AC: 3)
  ...
  - [x] Implement middleware for protected routes at `src/middleware.ts`
...
- Note: Next.js 16 shows middleware deprecation notice (middleware still works)

### File List
**Created:**
- `src/lib/auth/auth.config.ts` - NextAuth configuration with Credentials provider
- `src/lib/auth/auth.ts` - Auth utilities with verifyCredentials function
...
- `package.json` - Added next-auth and yup dependencies
